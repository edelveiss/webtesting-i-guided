...
describe("arithmetic functions", () => {
  describe("add()", () => {
    ...
    it("should throw an exception if the parms are not numbers", () => {
      ...
      expect(() => {
        add([1, 2, "red"]);
      }).toThrow(); // a string in an array
      expect(() => {
        add([1, 2, NaN]);
      }).toThrow(); // NaN in an array
      expect(() => {
        add({ prop: "prop" }, 2);
      }).toThrow(); // an object
    ...
    //
    it("add support for any number of arguments", () => {
  ...
  describe("static string tests1", () => {
    test("retrun value contains the word static", () => {
      expect(staticString()).toStrictEqual("this is a static string");
    });
  });
});
